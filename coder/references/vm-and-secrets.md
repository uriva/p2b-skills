...
- **Verify channel capabilities and tokens before testing:** Before claiming that a message flow is working, verify that the target bot actually has its WhatsApp for Business Token (`whatsappForBusinessToken`) or other necessary tokens configured in InstantDB. If a bot does not have an active token configured, proactively inform the user instead of letting the task fail silently.
