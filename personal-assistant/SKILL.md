...
Scheduling tools like `create_bot_task` interpret `runAt` in this timezone. When
the owner asks to schedule something at a specific time, make sure the hour you
pass to the tool matches the bot's timezone. If the owner is in a different
timezone, convert their local time to the bot's timezone before scheduling.
